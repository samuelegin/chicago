...
function Avatar({ name, avatar, size = 32, rank }) {
  ...
  if (ring) {
    return (
      <div className={`rounded-full bg-gradient-to-br ${ring} p-[2px] shrink-0`} style={{ width: size + 4, height: size + 4 }}>
        <div className="rounded-full bg-white p-[1.5px] w-full h-full">
          <div className="rounded-full bg-neutral-100 overflow-hidden flex items-center justify-center w-full h-full">
            {avatar
              ? <img src={avatar} alt={name} className="w-full h-full object-cover" />
              : <span className="font-bold text-neutral-400 select-none" style={{ fontSize: size * 0.38 }}>{(name || '?')[0].toUpperCase()}</span>
            }
          </div>
        </div>
      </div>
  ...
  return (
    <div className="rounded-full bg-neutral-100 border border-border overflow-hidden shrink-0 flex items-center justify-center"
...
export default function RightSidebar({ topUsers = [], currentUser }) {
  return (
    <aside className="hidden xl:flex flex-col w-[260px] shrink-0 h-screen overflow-y-auto py-6 px-4 gap-5 sticky top-0">
      ...
      {/* Suggestions */}
      <div className="bg-white border border-border rounded-xl p-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-bold text-neutral-400 tracking-wider uppercase">Suggested</p>
            ...
            <div key={user.id} className="flex items-center justify-between gap-2">
              <Link to={`/profile/${user.user_id}`} className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {user.rank && (
                    <p className="text-[11px] text-neutral-400 leading-tight">{RANK_LABEL[user.rank] || user.rank}</p>
                  )}
                </div>
              </Link>
        ...
        <div className="bg-white border border-border rounded-xl p-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-[11px] font-bold text-neutral-400 tracking-wider uppercase">Top Influence</p>
            </div>
            <Link to="/leaderboard" className="text-[11px] font-bold text-amber-600 hover:text-amber-700 transition-colors">
              Full list
            </Link>
            ...
            {topUsers.slice(0, 5).map((user, i) => (
              ...
              >
                <span className="text-base w-5 text-center shrink-0 leading-none">
                  {MEDAL[i] || <span className="text-[11px] font-bold text-neutral-300">{i + 1}</span>}
                </span>
      ...
      {/* Footer */}
      <p className="text-[10px] text-neutral-300 text-center px-2">
