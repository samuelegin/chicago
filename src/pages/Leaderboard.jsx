...
export default function Leaderboard() {
  ...
  return (
    <div className="max-w-[600px] mx-auto px-3 lg:px-4 py-6 space-y-4">
      {/* Header */}
      <div className="bg-gradient-to-r from-primary/5 via-card to-primary/5 border border-border rounded-3xl px-5 py-5 shadow-sm">
        <h1 className="text-base font-semibold tracking-wide text-foreground">Leaderboard</h1>
        <p className="text-sm text-muted-foreground mt-1">Top members by influence, activity, and staking</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-sm text-muted-foreground">No data yet</div>
        ) : (
          <div className="space-y-4">
                ...
                return (
                  <Link
                    ...
                    className={`flex flex-col items-center text-center rounded-3xl p-3 sm:p-4 transition-all ${
                      isTop
                        ? 'col-span-1 bg-gradient-to-br from-amber-500/10 via-card to-amber-400/5 border-2 border-amber-500/30 shadow-[0_12px_32px_rgba(251,191,36,0.12)] sm:shadow-[0_24px_60px_rgba(251,191,36,0.18)] scale-100 sm:scale-[1.04]'
                        : 'bg-card border border-border shadow-sm'
                    } ${isSecond ? 'translate-y-2 sm:translate-y-4' : ''} ${isThird ? 'translate-y-4 sm:translate-y-7' : ''}`}
                  >
                    <span className={`text-xs font-semibold tracking-[0.12em] uppercase ${isTop ? 'text-amber-500' : 'text-muted-foreground'}`}>
                      #{isTop ? 1 : idx === 0 ? 2 : 3}
                    </span>
                    <Avatar name={u.username} avatar={u.avatar_url} size={isTop ? 72 : 58} />
                    <p className={`text-sm font-semibold truncate mt-3 ${isTop ? 'text-foreground' : 'text-foreground'}`}>
                      {u.username}
                    </p>
                    <div className={`text-[12px] mt-2 ${isTop ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
                      {(u.cis_score || 0).toLocaleString()} influence
                    </div>
                    <div className={`text-[12px] mt-1 ${isTop ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
            ...
            {/* Positions 4 - 10 */}
            <div className="mt-6 sm:mt-3 bg-card border border-border rounded-sm divide-y divide-border">
              {users.slice(3, 10).map((u, i) => (
                ...
                >
                  <span className="text-xs font-semibold text-muted-foreground w-5 text-center shrink-0">{i + 4}</span>
                  ...
                  <div className="bg-card border border-border rounded-sm px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground">{(u.cis_score || 0).toLocaleString()} influence · {(u.clt_staked || 0).toLocaleString()} CLT</p>
                      </div>
                    </div>
